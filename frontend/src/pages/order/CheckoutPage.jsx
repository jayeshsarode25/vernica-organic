  ...
  return (
    <button
      ...
      className={`w-full text-left p-4 rounded-xl border-2 transition-all ${
        isSelected ? "border-gray-900 bg-gray-50 shadow-sm" : "border-gray-100 bg-white hover:border-gray-300"
      }`}
    >
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-start gap-3">
          ...
          >
            {isSelected && <span className="w-2 h-2 rounded-full bg-gray-900 block" />}
          </span>
          <div>
...
export default function CheckoutPage() {
  ...
  // ✅ Read discount from Redux cart state
  const { items: cartItems, discount } = useSelector((s) => s.cart);
  ...
  const handleSubmit = (e) => {
    ...
    if (!selectedAddress) {
      dispatch(setToast({ msg: "Please select a delivery address", type: "error" }));
      return;
    }
  ...
  // ✅ Calculate subtotal and apply discount from Redux
  ...
  const displayTotal = Math.max(subtotal - discount, 0);
    ...
    <div className="max-w-5xl mx-auto px-4 py-10 flex flex-wrap gap-6 items-start">
      ...
      <div className="w-72 shrink-0 bg-white rounded-xl shadow-sm border border-gray-100 p-6 sticky top-5">
        ...
        <div className="space-y-3">
          {cartItems.map((item) => {
            const name = item.productId?.title ?? item.productId?.name ?? item.name ?? "Product";
            const qty = item.qty ?? item.quantity ?? 1;
            const price = item.productId?.price?.amount ?? item.price?.amount ?? item.price ?? 0;
            return (
